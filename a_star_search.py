...
def tsp_heuristic(state, remaining_cities):
    ...
    for city in remaining_cities:
        cost = np.linalg.norm(city - state[-1])
        new_state = state + [city]
        new_remaining_cities = [c for c in remaining_cities if c != city]
        cost += tsp_heuristic(new_state, new_remaining_cities)
        if cost < min_cost:
            min_cost = cost
...
def a_star(start, city_coords):
    """
    Finds the shortest path that visits all the cities using A* algorithm.

    Args:
        start (np.ndarray): The starting city.
        city_coords (List[np.ndarray]): The list of city coordinates.

    Returns:
        tuple: The shortest path that visits all the cities and its length.
    """

    visited = [start]
    path = [start]
    remaining_cities = [c for c in city_coords if c not in visited]
...
best_astar = []
for i in range(k):
    astar_path, astar_distance = a_star(best_path[0], best_path)
    best_astar.append([best_path, best_distance])

res_path = []
res_distance = sum([city[1] for city in best_cluster])
for i in range(k):
    if i == k-1:
        break
